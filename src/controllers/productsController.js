...
export const updateProduct = async (req, res) => {
    try {
        ...
        const product = await productsService.getProductsBy({ _id: pId });
        if (!product) {
            return res.status(404).json({msg: `Error updating product no encontrado: ${pId}`});
        }
        
        const updatedProduct = await productsService.updateProduct(pId, modif);
        if (updatedProduct)
            return res.json({msg: "Updated product", updatedProduct})
